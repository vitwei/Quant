...
data=pd.read_feather('Database/ktest.feather')
...
data['str']=np.sqrt(((data['1W']**2).cumsum())/(data.index+1))
...
measurements = np.array(data[['1d','var']])
...
data['Bpro']=res[2]
data['Bpro']=res['cdf']
data.to_feather('Database/filter_res.feather')
