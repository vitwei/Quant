...
class st4(bt.Strategy):
    def __init__(self):
        self.dataclose = self.data0.close
        self.distribution = self.data0.pro
        self.profit_target = 0.04  # 5%的收益目标
        self.stop_target = -0.01
        ...
        #self.log=pd.DataFrame(columns=['日期','行为','价格','信号','平均收益','平均日收益'])
    def next(self):
        if self.distribution[0] < 40 and self.stop_mark==False:   
            buy_price = self.data0.close[0]  # 获取当前的买入价格
            if self.buy_times==-1:
                self.buy()
                buy_price = self.data0.close[0]
                buy_transaction = pd.DataFrame({'日期':[self.data0.datetime.datetime()],'成本': [buy_price], 'day': [1]})
                self.buy_transaction = pd.concat([self.buy_transaction, buy_transaction])
                self.buy_times+=1
                #log = pd.DataFrame({'日期':[self.data0.datetime.datetime()],'行为': ['买入'],'价格': [buy_price],'信号':[self.distribution[0]]})
                #self.log = pd.concat([self.log, log])
            if self.buy_times!=-1 and self.total_return>0 and self.buy_times<3:
                self.buy()
                buy_price = self.data0.close[0]
                buy_transaction = pd.DataFrame({'日期':[self.data0.datetime.datetime()],'成本': [buy_price], 'day': [1]})
                self.buy_transaction = pd.concat([self.buy_transaction, buy_transaction])
        ...
        if self.position:
            ...
            if not self.buy_transaction.empty and not self.buy_transaction['day'].isnull().all():
                average_return = self.total_return / int(self.buy_transaction['day'].mean())
            else:
            ...
            if self.total_return>self.profit_target:
                ...
                #log = pd.DataFrame({'日期':[self.data0.datetime.datetime()],'行为': ['卖出'],'价格': [self.data.close[0]],'信号':['5%到手'],'平均收益':[self.total_return],'平均日收益':[average_return]})
                #self.log = pd.concat([self.log, log])
            if  average_return < self.daily_return_threshold and self.buy_transaction['day'].max()>=60:
                self.close()
                self.buy_times=-1
                self.buy_transaction =pd.DataFrame(columns=['成本','day'])
                #log = pd.DataFrame({'日期':[self.data0.datetime.datetime()],'行为': ['卖出'], '价格': [self.data.close[0]],'信号':['日均较低'],'平均收益':[self.total_return],'平均日收益':[average_return]})
                #self.log = pd.concat([self.log, log])
            if self.total_return <=self.stop_target:
                self.close()
                self.buy_times=-1
                self.buy_transaction=pd.DataFrame(columns=['成本','day'])
                #self.stop_mark=True
                #log = pd.DataFrame({'日期':[self.data0.datetime.datetime()],'行为': ['卖出'], '价格': [self.data.close[0]],'信号':['止损'],'平均收益':[self.total_return],'平均日收益':[average_return]})
                #self.log = pd.concat([self.log, log])
            if self.distribution[0]>70:
                    self.close()
                    self.buy_times=-1
                #log = pd.DataFrame({'日期':[self.data0.datetime.datetime()],'行为': ['卖出'], '价格': [self.data.close[0]],'信号':['止损分布'],'平均收益':[self.total_return],'平均日收益':[average_return]})
            if not self.buy_transaction.empty:
                self.buy_transaction['day']+=1
            #self.log.to_excel('log.xlsx',index=None)

class st5(bt.Strategy):
    def __init__(self):
        self.dataclose = self.data0.close
        self.distribution = self.data0.pro
        self.profit_target = 0.04  # 5%的收益目标
